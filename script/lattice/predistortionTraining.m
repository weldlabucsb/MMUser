...
numFeatures = 1;     % 1 Channel (Target Optical Amplitude)
...
for iter = 1:maxIter
    ...
    for b = 1:batchSize
        ...
        predictedAwg1 = mlWf2realWf(predict(net1, wf1.Sample),voltageRange);
        predictedAwg2 = mlWf2realWf(predict(net2, wf2.Sample),voltageRange);
        ...
        % Align time delay, downsample, and store
        [scopeMl1, awgMl1] = preprocessData(scopeRaw(1,:), awgWf1.Sample, scopeSr, awgSr, mlSr,delay1);
        xDataset1{end+1} = scopeMl1;
        yDataset1{end+1} = realWf2MlWf(awgMl1(1,:),voltageRange);

        [scopeMl2, awgMl2] = preprocessData(scopeRaw(2,:), awgWf2.Sample, scopeSr, awgSr, mlSr,delay2);
        xDataset2{end+1} = scopeMl2;
        yDataset2{end+1} = realWf2MlWf(awgMl2,voltageRange);

        % Calculate RMSE for convergence tracking
        min_len = min(length(scopeMl1), length(wf1.Sample));
        y_true = scopeMl1(1:min_len);
        y_pred = wf1.Sample(1:min_len);
        waveform_rmse = sqrt(mean((y_true - y_pred).^2));
        batchError1 = batchError1 + waveform_rmse;

        min_len = min(length(scopeMl2), length(wf2.Sample));
        y_true = scopeMl2(1:min_len);
        y_pred = wf2.Sample(1:min_len);
        waveform_rmse = sqrt(mean((y_true - y_pred).^2));
    ...
    net2 = trainNetwork(xDataset1, yDataset1, net2.Layers, options);
end

%% 5. Save the Final Model
disp('Saving the converged model...');
save('TeO2_AOM_Predistortion_Model.mat', 'net1', 'awgSr', 'scopeSr', 'duration');

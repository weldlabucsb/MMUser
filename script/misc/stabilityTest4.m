...
%% MRC
t = readtable("BeamStab_2024-12-21T19-07-32.csv");
t0 = t.ms * 1e-3;
v0 = t.I1_V_;
x = t.X1_V_;
y = t.Y1_V_;
...
for ii = 1:numel(tiList)
    idx = t0 >= tiList(ii) & t0 <= (tiList(ii)+dt) & v0 > 1.45;
    idx = find(idx);
    while ~isempty(idx) && (v0(idx(1)) - v0(idx(1)-1)) < 1.3
        idx(1) = [];
    end
    while ~isempty(idx) && (v0(idx(end)) - v0(idx(end)+1)) < 1.3
        idx(end) = [];
    end
    tMRC(ii) = mean(t0(idx));
    vMRC(ii) = mean(v0(idx));
